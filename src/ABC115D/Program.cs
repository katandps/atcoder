...
namespace ABC115D
{
    ...
    class Solver
    {
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                patties[i + 1] = patties[i] * 2 + 1;
                height[i + 1] = height[i] * 2 + 3;
            }

            Console.WriteLine(calc(N, 0, X));
        }

        private long[] patties;
        private long[] height;

        long calc(int curLev, long p, long cursor)
        {
            if (curLev == 1) return p + Math.Max(0, Math.Min(3, cursor - 1));

            var h = height[curLev];
            if (cursor < h / 2)
            {
                return calc(curLev - 1, p, cursor - 1);
            }

            if (cursor == h / 2)
            {
                return p + patties[curLev - 1];
            }

            if (cursor == h / 2 + 1)
            {
                return p + patties[curLev - 1] + 1;
            }

            return calc(curLev - 1, p + patties[curLev - 1] + 1, cursor - (h + 1) / 2);
        }
    }
}