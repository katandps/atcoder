...
namespace ABC160D
{
    ...
    class Solver
    {
        ...
        public void Solve()
        {
            ...
            {
                long ans = 0;
                if (i < loopSize / 2) ans += loopSize;
                if (loopSize % 2 == 0 && i == loopSize / 2) ans += loopSize / 2;
                if (loopSize % 2 == 1 && i == loopSize / 2) ans += loopSize;
                ...
                for (int j = 1; j <= N; j++)
                {
                    if (j < X)
                    {
                        ...
                        //X側からY側を見る Y側からX側は見ない
                        if (i + j > X + 1 && i + j <= N - (Y - X) + 1) ans++;
                        if (j + i > X)
                        {
                            //X側からループを見る
                            var rest = i + j - X;
                            if (rest < loopSize / 2) ans += 2;
                            if (rest == loopSize / 2)
                            {
                                if (loopSize % 2 == 0) ans += 1;
                                else ans += 2;
                            }
                        }
                    }
