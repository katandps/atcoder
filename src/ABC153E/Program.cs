...
namespace ABC153E
{
    ...
    class Solver
    {
        public Solver()
        {
            ...
            input.Longs(ref H, ref N);
            input.LongsArray(N, ref A, ref B);
        }
        ...
        public void Solve()
        {
            //DP[HP] ＝ そのHPを削るのに必要な最小の魔力
            long[] dp = new long[H + 1];
            for (int i = 1; i < H + 1; i++)
            {
                dp[i] = Int64.MaxValue / 100;
            }
            ...
            for (int i = 0; i < N; i++)
            {
                for (var j = A[i]; j <= H; j++)
                {
                    dp[j] = Math.Min(dp[j - A[i]] + B[i], dp[j]);
                }

                if (A[i] <= H)
                {
                    var m = Int32.MaxValue;
                    for (var j = H - A[i]; j < H; j++)
                    {
                        m = Math.Min(m, (int)dp[j]);
                    }
                    dp[H] = Math.Min(m + B[i], dp[H]);
                }
                else
                {
                    dp[H] = Math.Min(B[i], dp[H]);
                }
