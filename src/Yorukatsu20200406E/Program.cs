...
using System.IO;
using System.Linq;
using static Yorukatsu20200406E.Input;

namespace Yorukatsu20200406E
{
    static class Input
    {
    ...
    class Solver
    {
        public void Solve()
        {
            ...
            long[][] leftDp = new long[n + 1][];
            long[][] rightDp = new long[n + 1][];
            leftDp[0] = new long[t];
            rightDp[n] = new long[t];

            for (int li = 0; li < n; li++)
            {
                int ri = n - li - 1;
                leftDp[li + 1] = new long[t];
                rightDp[ri] = new long[t];

                for (int i = t - 1; i >= 0; i--)
                {
                    if (i - a[li] < 0)
                    {
                        leftDp[li + 1][i] = leftDp[li][i];
                    }
                    else
                    {
                        leftDp[li + 1][i] = Math.Max(leftDp[li][i], leftDp[li][i - a[li]] + b[li]);
                    }
                }
                ...
                for (int i = t - 1; i >= 0; i--)
                {
                    if (i - a[ri] < 0)
                    {
                        rightDp[ri][i] = rightDp[ri + 1][i];
                    }
                    else
                    {
                        rightDp[ri][i] = Math.Max(rightDp[ri + 1][i], rightDp[ri + 1][i - a[ri]] + b[ri]);
                    }
                }
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < t; j++)
                {
                    ans = Math.Max(ans, keep + leftDp[i][j] + rightDp[i + 1][t - j - 1]);
                }
