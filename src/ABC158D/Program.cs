...
namespace ABC158D
{
    ...
    class Solver
    {
        ...
        {
            // クエリを後ろから処理する
            // 前につけるか後ろにつけるかは反転が来たらひっくり返る

            int FRONT = 0;
            int LAST = 1;
            List<char>[] list = new List<char>[2];
            for (int i = 0; i < 2; i++)
            {
            ...
            foreach (char c in list[FRONT])
            {
                Console.Write(c);
            }

            var s = rev ? S.Reverse() : S.ToCharArray();
            foreach (char c in s)
            {
                Console.Write(c);
            }
