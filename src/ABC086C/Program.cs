...
namespace ABC086C
{
    ...
    class Solver
    {
        ...
        {
            @in(out N);
            @in(N, out t, out x, out y);

            // t x y
            long[] before = {0, 0, 0};
            for (int i = 0; i < N; i++)
            {
                var d = Math.Abs(x[i] - before[1]) + Math.Abs(y[i] - before[2]);
                var tdiff = t[i] - before[0];
                if (tdiff < d || tdiff % 2 != d % 2)
                {
                    Console.WriteLine("No");
                    return;
                }

                before = new[] {t[i], x[i], y[i]};
            }
