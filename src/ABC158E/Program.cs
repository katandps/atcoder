...
namespace ABC158E
{
    ...
    class Solver
    {
        public Solver()
        {
            ...
            input.Longs(ref N, ref P);
            var s = input.String();
            S = new int[N];
            for (int i = 0; i < N; i++)
            {
                S[i] = int.Parse(s[i].ToString());
            }
        }
        ...
        public void Solve()
        {
            long[] memo = new long[P];
            long[] tmp1 = new long[P];
            
            HashSet<long>[] set = new HashSet<long>[P];

            for (int i = 0; i < P; i++)
            {
                set[i] = new HashSet<long>();
            }
