...
using System.IO;
using System.Linq;
using static ABC105_20200320D.Input;

namespace ABC105_20200320D
{
    static class Input
    {
        /// <summary>
    ...
    class Solver
    {
        ...
        public void Solve()
        {
            ...
            Dictionary<long, long> reminders = new Dictionary<long, long>();
            for (int i = 1; i <= N; i++)
            {
                long r = sum[i] % M;
                if (!reminders.ContainsKey(r)) reminders.Add(r, 0);
                reminders[r]++;
            }

            long ans = 0;
            if (reminders.ContainsKey(0)) ans += reminders[0]; // l =１のパターン
            foreach (var l in reminders)
            {
                long v = l.Value;
                // １番目からの和のmodMが同じになるとき、後半の和から前半の和を除いた区間の余りは0になる
                // 余りxになるポイントが5こあるとき、そのような区間の数は5C2
                ans += v * (v - 1) / 2;
            }
