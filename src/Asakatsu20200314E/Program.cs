...
namespace Asakatsu20200314E
{
    ...
    class Solver
    {
        ...
        public void Solve()
        {
            Dictionary<char, List<long>> pos = new Dictionary<char, List<long>>();
            for (char i = 'a'; i <= 'z'; i++)
            {
                pos.Add(i, new List<long>());
            }
            ...
            for (int i = 0; i < t.Length; i++)
            {
                ...
                int l = (int) Search(pos[cur], s_i);
                long next_s_i;
                if (l == pos[cur].Count)
                {
                    next_s_i = pos[cur].First();
                    ans += next_s_i + s.Length - s_i;
                    s_i = next_s_i;
                    continue;
                }
        ...
        long Search(List<long> list, long min)
        {
            long ng = -1;
            long ok = list.Count;
        ...
        bool IsOK(List<long> list, long key, long min)
        {
            return list[(int) key] > min;
        }
    }
}