...
using System.IO;
using System.Linq;
using static Asakatsu20200321E.Input;

namespace Asakatsu20200321E
{
    static class Input
    {
        /// <summary>
    ...
    class Solver
    {
        ...
        public void Solve()
        {
            ...
            // あとi回でゴールに辿り着く出目のうち、最も大きい数字のリスト
            var dp = new List<long>();
            long cur = N;
            while (cur > 0)
            {
                //なるべく多く進む
                // 現在curにいるとき、最大限進んだ位置は max(cur - M, 0)
                long t = -1;
                for (int j = (int) Math.Max(cur - M, 0); j < cur; j++)
                {
                    if (S[j] == '1') continue;
                    t = j;
                    break;
