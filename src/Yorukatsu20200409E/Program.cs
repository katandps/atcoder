...
using System.IO;
using System.Linq;
using static Yorukatsu20200409E.Input;

namespace Yorukatsu20200409E
{
    static class Input
    {
    ...
    class Solver
    {
        public void Solve()
        {
            ...
            var p = new Permutation<int>(Enumerable.Range(2, n - 1).ToList());

            int ans = 0;
            foreach (List<int> pattern in p.permutation)
            {
                pattern.Add(1);
                bool dame = false;
                for (int i = 1; i < pattern.Count; i++)
                {
                    if (!map[pattern[i - 1]][pattern[i]])
                    {
                        dame = true;
                    }
                }
