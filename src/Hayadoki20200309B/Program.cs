...
namespace Hayadoki20200309A
{
    ...
    class Solver
    {
        ...
        public Solver()
        {
            ...
            for (long i = 0; i < N; i++)
            {
                ...
                S[i] = new Shop(t[0], int.Parse(t[1]), (int) i);
            }
        }

        public void Solve()
        {
            Array.Sort(S, new ShopComparer());
            foreach (Shop shop in S)
            {
                Console.WriteLine(shop.Number);
            }
        }
    }
    ...
    {
        int IComparer<Shop>.Compare(Shop A, Shop B)
        {
            var a = A.Name;
            var b = B.Name;
            if (a == b)
