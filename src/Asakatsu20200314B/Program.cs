...
namespace Asakatsu20200314B
{
    ...
    class Solver
    {
        ...
        public Solver()
        {
            input = new Input();
            input.Long(out N);
            input.Long(N, out t, out x, out y);
        }
        ...
        public void Solve()
        {
            long[] before = {0, 0, 0}; //t=0 x=0 y=0
            for (long i = 0; i < N; i++)
            {
                long norm = Math.Abs(x[i] - before[1]) + Math.Abs(y[i] - before[2]);
                if (norm > t[i] - before[0] || norm % 2 != (t[i] - before[0]) % 2)
                {
