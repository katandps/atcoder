...
using System.IO;
using System.Linq;
using static Asakatsu20200325D.Input;

namespace Asakatsu20200325D
{
    static class Input
    {
    ...
    class Solver
    {
        private long H, W, A, B;

        public void Solve()
        {
            Cin(out H, out W, out A, out B);
            if (B > H / 2 || A > W / 2)
            {
                Console.WriteLine(-1);
                return;
            ...
            for (int i = 0; i < H; i++)
            {
                Console.WriteLine(new string(HW[i]));
            }
        }
    }
}